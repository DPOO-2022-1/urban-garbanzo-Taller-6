...
import uniandes.dpoo.taller1.modelo.YaExisteNombreEXP;
import uniandes.dpoo.taller1.modelo.CategoriaNoExisteEXP;
...
public class InterfazLibreria extends JFrame
{
	...
	public void cargarArchivos(File archivo_categorias, File archivo_libros)
	{
		try
		{
			...
			if(libreria.catAnadidos.size()>0)
			{
				for (Categoria categoria : libreria.catAnadidos)
				{
	
					ArrayList<Libro> librosCat = categoria.darLibros();
					String mensaje = "Ha sido agregada la categoria " + categoria.darNombre()+ " y tiene " + librosCat.size()+ " libro(s)";
 					JOptionPane.showMessageDialog(this, mensaje, "Categorias desconocidas",
	...
	public void renombrarCategorias()
	{
		String categoria = JOptionPane.showInputDialog(this, "Escriba la categor�a que quiere renombrar", "categoria");
		String renombre = JOptionPane.showInputDialog(this, "Escriba el nombre que quiere poner", "nuevo nombre");
		
		if (categoria != null && renombre != null)
		{
			try {
				libreria.renombrarCategorias(categoria, renombre);
			}
			catch(YaExisteNombreEXP ex) {
				JOptionPane.showMessageDialog(this, ex.getMessage(), "Renombre Existente",
						JOptionPane.ERROR_MESSAGE);
				
			}
			catch(CategoriaNoExisteEXP e) {
				JOptionPane.showMessageDialog(this, e.getMessage(), "Categor�a inexistente",
						JOptionPane.ERROR_MESSAGE);
			}
	...
	{
		String autores = JOptionPane.showInputDialog(this, "Escriba los nombres de los autores cuyos libros quiere borrar, escribalos juntos seguidos de comas");
